...
namespace SnapLink_API.Controllers
{
    ...
    public class PhotoDeliveryController : ControllerBase
    {
        ...
        // POST: api/PhotoDelivery/upload-photos
        [HttpPost("upload-photos")]
        public async Task<ActionResult<ApiResponse<PhotoDeliveryResponse>>> UploadPhotos([FromBody] UploadPhotosRequest request)
        {
            try
            {
                var result = await _photoDeliveryService.UploadPhotosAsync(request);
                return Ok(new ApiResponse<PhotoDeliveryResponse>
                {
                    Error = 0,
        ...
        // POST: api/PhotoDelivery/mark-delivered
        [HttpPost("mark-delivered")]
        public async Task<ActionResult<ApiResponse<PhotoDeliveryResponse>>> MarkAsDelivered([FromBody] MarkAsDeliveredRequest request)
        {
            try
            {
                var result = await _photoDeliveryService.MarkAsDeliveredAsync(request);
                return Ok(new ApiResponse<PhotoDeliveryResponse>
                {
                    Error = 0,
                    Message = "Photo delivery marked as delivered successfully",
                    Data = result
                });
            }
            catch (InvalidOperationException ex)
            ...
            catch (Exception ex)
            {
                ...
                {
                    Error = -1,
                    Message = "An error occurred while marking photo delivery as delivered",
                    Data = null
                });
            }
        }
